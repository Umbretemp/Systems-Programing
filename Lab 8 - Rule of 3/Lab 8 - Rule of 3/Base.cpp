...
void Base::setName(const char* name)
{
	...
	// new an array of that size for _name to point to
	_name = new char[length];
	// copy the chars from name to _name
	strcpy_s(_name, length, name); // copies chars from name to _name until sees a null terminator
}
